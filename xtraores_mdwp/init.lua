...
minetest.register_craftitem("xtraores_mdwp:shuriken", {
	description = "platinum shuriken|deals 10 dmg)",
	range = 0,
	inventory_image = "xtraores_shuriken.png",
	on_use = function(itemstack, user, pointed_thing)
		...
		if pos and dir then
			pos.y = pos.y + 1.5
			local obj = minetest.add_entity(pos, "xtraores_mdwp:se")
			if obj then
...
local XTRAORES_MDWP_SE = {
	physical = false,
	timer = 0,
	visual = "mesh",
	mesh = "shuriken.x",
	visual_size = {x=1.2, y=1.2,},
	textures = {'xtraores_platinum_brick.png'},
	lastpos= {},
	collisionbox = {0, 0, 0, 0, 0, 0},
}
XTRAORES_MDWP_SE.on_step = function(self, dtime)
	self.timer = self.timer + dtime
	local pos = self.object:getpos()
	local node = minetest.get_node(pos)
	...
	if self.timer > 0.04 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "xtraores_mdwp:se" and obj:get_luaentity().name ~= "__builtin:item" then
					local damage = 10
					obj:punch(self.object, 1.0, {
						full_punch_interval = 1.0,
						damage_groups= {fleshy = damage},
	...
	if self.lastpos.x ~= nil then
		if minetest.registered_nodes[node.name].walkable then
			if not minetest.setting_getbool("creative_mode") then
				minetest.add_item(self.lastpos, "xtraores_mdwp:shuriken")
			end
			minetest.sound_play("default_dig_cracky", {pos = self.lastpos, gain = 0.8})
			self.object:remove()
		end
...
minetest.register_entity("xtraores_mdwp:se", XTRAORES_MDWP_SE )

minetest.register_craft({
	output = 'xtraores_mdwp:shuriken 15',
	recipe = {
